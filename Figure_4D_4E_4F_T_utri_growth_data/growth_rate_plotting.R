...
output_prefix <- "Anox_original_14d"  # Change this to your desired prefix without spaces
...
# Read the data
data <- read.csv("cell_growth_data_anox_long.csv")  # Ensure this path is correct
...
# 2. Grouped bar plot with data points and trend lines
bar_plot <- ggplot(data_summary, aes(x = Day, y = Mean_Count, fill = Cell_Type, group = Cell_Type)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.8) +
  geom_errorbar(aes(ymin = Mean_Count - SE, ymax = Mean_Count + SE), 
                position = position_dodge(0.9), width = 0.25) +
  geom_point(data = data_long, aes(y = Cell_Count, group = Cell_Type),
             position = position_dodge(width = 0.9), size = 1.5, shape = 21, fill = "white") +
  geom_line(aes(color = Cell_Type), position = position_dodge(width = 0.9), size = LINE_WIDTH) +
  scale_fill_manual(values = color_palette) +
  scale_color_manual(values = color_palette) +
  theme_times_new_roman() +  # Changed from theme_minimal()
  labs(title = "Cell Count Comparison by Day",
       x = "Days",
       y = "Cell Count") +
  theme(
...
# 3. Box plots
box_plot <- ggplot(data_long, aes(x = Day, y = Cell_Count, fill = Cell_Type)) +
  geom_boxplot() +
  scale_fill_manual(values = color_palette) +
  theme_times_new_roman() +  # Changed from theme_minimal()
  labs(title = "Distribution of Cell Counts by Day",
       x = "Days",
       y = "Cell Count") +
  theme(
    plot.title = element_text(size = PLOT_TITLE_SIZE),
...
box_plot_with_stats <- box_plot +
  geom_signif(comparisons = list(c("D0", "D5")),
              map_signif_level = TRUE,
              y_position = max(data_long$Cell_Count) * 1.1)

print(box_plot_with_stats)

# 4. Faceted plots with connected points
facet_plot <- ggplot(data_summary, aes(x = Day, y = Mean_Count, color = Cell_Type, group = Cell_Type)) +
  geom_line(size = LINE_WIDTH) +
  geom_errorbar(aes(ymin = Mean_Count - SE, ymax = Mean_Count + SE), width = 0.2) +
  geom_point(size = POINT_SIZE) +
  ...
  scale_color_manual(values = color_palette) +
  theme_times_new_roman() +  # Changed from theme_minimal()
  labs(title = "Average Cell Growth by Type",
       x = "Days",
       y = "Average Cell Count") +
  theme(
    plot.title = element_text(size = PLOT_TITLE_SIZE),
...
ggsave(generate_filename("bar_plot"), bar_plot, width = PLOT_WIDTH, height = PLOT_HEIGHT, device = cairo_pdf)
ggsave(generate_filename("box_plot"), box_plot, width = PLOT_WIDTH, height = PLOT_HEIGHT, device = cairo_pdf)
ggsave(generate_filename("box_plot_with_stats"), box_plot_with_stats, width = PLOT_WIDTH, height = PLOT_HEIGHT, device = cairo_pdf)
ggsave(generate_filename("facet_plot"), facet_plot, width = PLOT_WIDTH, height = PLOT_HEIGHT, device = cairo_pdf)
